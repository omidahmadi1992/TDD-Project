...
public class GetPrimaryAccountTest extends ApiTestsBase {
    ...
    public void validateGetAccountNotExist(){
        Response response = getDefaultRequest().queryParam("primaryPersonId",202020).when().get(EndPoints.GET_PRIMARY_ACCOUNT.getValue())
                .then().statusCode(404).extract().response();
        ExtentTestManager.getTest().info(response.asPrettyString());
        String errorMessage = response.body().jsonPath().getString("errorMessage");
        ExtentTestManager.getTest().info(response.asPrettyString());
        Assert.assertEquals(errorMessage, "Account with id 202020 not exist");
    }

    //Retrieve the Latest primary Person from database call API /get-primary-account
